...
export default function StoreGrid({ storePage, products, categories, lang }: any) {
  ...
  return (
    <div className='w-full flex flex-col gap-25 pb-25'>
      ...
      <section className="andes-contenido flex flex-col gap-24">

  {categories?.map((cat: any) => {
    const categoryProducts = products.filter(
      (p: any) => p.category?._ref === cat._id
    );

    if (!categoryProducts.length) return null;

    return (
      <div key={cat._id} className="flex flex-col gap-10">
        ...
        <div className="relative w-full">
          ...
          <div className="flex gap-10 overflow-x-auto scrollbar-none snap-x snap-mandatory px-4">
            ...
            {categoryProducts.map((product: any) => (
              <div
                key={product._id}
                className="min-w-[260px] max-w-[260px] snap-center flex-shrink-0 flex flex-col items-center text-center group"
              >
                {/* IMAGE */}
                ...
                <Link
                  ...
                  className="mt-3 text-sm opacity-0 group-hover:opacity-100 transition"
                >
